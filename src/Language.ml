...
module Expr =
  struct
    ...
    *)                                                       
    ...
    | Elem (a, indx) -> 
        let (statement, input, output, Some value_a) as sior = eval env sior a in 
        let (statement, input, output, Some value_indx) as sior = eval env sior indx in 
    ...
    | Call (name, param) ->
        let (sior', param') = List.fold_left 
      	(fun (sior, cv) expr -> let (statement, input, output, Some v) as sior' = eval env sior expr in (sior', cv @ [v])) (sior, [])
     	param in
