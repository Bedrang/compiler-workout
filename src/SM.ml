...
let run p i =
  ...
  | Expr.Call (name, param) ->
    List.concat (List.map exprcompile (List.rev param)) @ [CALL (name, List.length param, false)]
...
let rec compile' stmt =
  match stmt with
   ...
   |Stmt.Call (name, args) -> 
    List.concat (List.map exprcompile args) @ [CALL (name, List.length args, true)]
   |Stmt.Return expr -> (
      match expr with
      | None -> [RET false]
      | Some expr -> exprcompile expr @ [RET true]
    )
    | _ -> failwith "Undefined Behavior"
    in
   let compile (name, (args, loc, body)) = [LABEL name; BEGIN (name, args, loc)] @ compile' body @ [END] in
